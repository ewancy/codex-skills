...
中文说明：当用户要求“下主任务/子任务”时，先按需求关联的执行/版本创建主任务，再按确认的角色创建子任务；排期必须避开周天。

- 主任务默认为产品任务，标题使用需求标题，默认指派给当前产品负责人/当前用户，截止日期按用户给定日期。
- 子任务标题默认：`<主任务标题> 后端`、`<主任务标题> 前端`、`<主任务标题> 测试`、`<主任务标题> 美术`。
- 测试子任务截止日期默认与主任务截止日期一致。
- 后端、前端子任务截止日期默认比测试提前 2 天；如果计算结果是周天，向前调整到非周天日期。
- 美术子任务只有用户确认需要时才创建；截止日期默认比前后端再提前 1-2 天，按工作量判断，并同样避开周天。
- AI 项目后端子类型默认选择 `AI及禅道`；平台部项目多数为 `账服`。如果涉及客服总台、客服后台、EGC 等子类型，创建前先向用户确认。
- 常见负责人账号：后端陈立强 `chenlq`，前端陈杰 `chenjie`，测试陈宇楠 `chenyn`，美术张倩文 `zhangqw`。
